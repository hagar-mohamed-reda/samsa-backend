<table class="table table-bordered default-datatable" id="table" >
    <thead>
        <tr>
            <th scope="col">ID</th>
            <th scope="col">{{ __('qualification') }}</th>
            <th scope="col">{{ __('qualification grade') }}</th>
            <th scope="col">{{ __('notes') }}</th>
            <th scope="col"> </th>
        </tr>
    </thead>
    <tbody>
        @php $i = 1 @endphp
        @foreach ($qualifications as $row)
        <tr>
            <th scope="row">{!! $row->id !!}</th>
            <td>{!! $row->name !!}</td>
            <td>{!! $row->grade !!}</td>
            <td>{!! $row->notes !!}</td>
            <td>
                <div class="btn-group" role="group" aria-label="Basic example">

                    @permission('qualifications_update')
                    <a href="{{route('qualifications.edit', $row->id)}}">
                        <div class="col-md-4 col-sm-6 col-12">
                            <div class="fonticon-wrap"><i class="fa fa-pencil"></i></div>
                        </div>
                    </a>
                    @endpermission

                    @permission('qualifications_delete')
                    <a onclick="Delete({{'deleteForm'.$row->id}})" id="type-warning">
                        <div class="col-md-4 col-sm-6 col-12">
                            <div class="fonticon-wrap"><i class="fa fa-trash-o"></i></div>
                        </div>
                    </a>
                    @endpermission
                    <form id="deleteForm{{$row->id}}" action="{{route('qualifications.destroy',$row->id)}}" method="post">
                        @method("DELETE")
                        @csrf
                    </form>
                </div>
            </td>
        </tr>
        @endforeach
    </tbody>
</table>
