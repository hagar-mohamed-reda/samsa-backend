    ...
    <img class="w3- w3-block imageView" 
        @if (isset($application))
            @if($application->personal_photo)
                src="{{ url('/') }}/{{ $application->personal_photo }}"
            @endif
...
<!--                {!! Form::select('qualification_id',Modules\Settings\Entities\Qualification::pluck('name','id'),null,['class'=>'custom-select','id'=>'customSelect','placeholder' =>  __('select qualification'), 'v-model="application.qualification_id"', 'required' => 'required'   ])!!}-->
               ...
               <select name="qualification_id" class="form-control w3-round" 
                required="required"  
                v-model="application.qualification_id">
                    @foreach(Modules\Settings\Entities\Qualification::get() as $item)
                    <option value="{{ $item->id }}" onClick = "@if($item->id == 4) showAzharDegree() @else hideAzharDegree()  @endif">{{ $item->name }}</option>
                    @endforeach
                </select>
        ...
        <tr>
            ...
            <td>   
                <select name="qualification_types_id"  class="form-control w3-round qualification_type_id qualification_types_id" 
                required="required"
                v-model="application.qualification_type_id" 
        ...
        <tr style="display: none" id="azharDegree">
            <td><label for="address">{{ __('total religious scores') }}*</label></td>
            <td> 
           {!! Form::text('azhar_grade',null,['class'=>'form-control w3-round','id'=>'grade','placeholder'=> __('total religious scores') ])!!}
                @error('address')
                <span class="text-danger"> {!! $message !!}</span>
                @enderror
                <div class="help-block"></div>
                    ...
                    <li class="d-inline-block mr-2">
                        <fieldset>
                            <div class="vs-radio-con" onchange="showDiv()">
                                <input type="radio" name="gender" @if(isset($application) && $application->gender == 1)checked="" @endif value="1">
                                <span class="vs-radio">
                    ...
                    <li class="d-inline-block mr-2">
                        <fieldset>
                            <div class="vs-radio-con" onchange="hidDiv()" >
                                <input type="radio" name="gender" @if(isset($application) && $application->gender == 0 )checked="" @endif  value="0">
                                <span class="vs-radio">
                                    <span class="vs-radio--border"></span>
                                    <span class="vs-radio--circle"></span>
                                </span>
        ...
        <tr>
            ...
            <ul class="list-unstyled mb-0">
                    ...
                    <li class="d-inline-block mr-2">
                        <fieldset>
                            <div class="vs-radio-con">
                                <input type="radio" name="religion" value="1" @if(isset($student) && $student->religion == 1 ) checked="checked" @endif>
                                <span class="vs-radio">
                                    <span class="vs-radio--border"></span>
                                    <span class="vs-radio--circle"></span>
                                </span>
                    ...
                    <li class="d-inline-block mr-2">
                        <fieldset>
                            <div class="vs-radio-con">
                                <input type="radio" name="religion" value="0" @if(isset($student) && $student->religion == 0 ) checked="checked" @endif>
                                <span class="vs-radio">
                                    <span class="vs-radio--border"></span>
                                    <span class="vs-radio--circle"></span>
                                </span>
...
<div class="row parent_info" >
    <table class="table" > 
        ...
        <tr>
         ...
         <tr>
            <td><label class="label-r" for="parent_relation">{{ __('parent relationship') }}</label></td>
            <td>  
                {!! Form::select('relative_relation_id',App\RelativeRelation::pluck('name','id'),null,['class'=>'custom-select','id'=>'customSelect','placeholder' => __('select parent relationship')])!!}
                @error('relative_relation_id')
                <span class="text-danger"> {!! $message !!}</span>
                @enderror
            </td>
        </tr>
                ...
                <div class="row" >
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <img 
                            ...
                            @if (isset($application))
                            @if (Modules\Adminsion\Entities\StudentRequiredDocument::where('application_id', $application->id)->where('required_document_id', $item->id)->first())
                            src="{{ optional(Modules\Adminsion\Entities\StudentRequiredDocument::where('application_id', $application->id)->where('required_document_id', $item->id)->first())->path }}"
                            @endif
                            @endif
...
//    }
    ...
    $(document).ready(function(){
        ...
        app.application.qualification_id = '{{ isset($application)? $application->qualification_id : "" }}';
    });
    var app = new Vue({
        el: '#app',
        data: {
            application: { 
//                level_id: '{{ isset($application)? $application->level_id : "" }}',
//                level_name: '{{ isset($application)? optional($application->level)->name : "" }}',
                qualification_type_id: '{{ isset($application)? $application->qualification_types_id : "" }}',
                qualification_id: '{{ isset($application)? $application->qualification_id : "" }}',
                country_id: '{{ isset($application)? $application->country_id : "" }}',
                government_id: '{{ isset($application)? $application->government_id : "" }}',
                city_id: '{{ isset($application)? $application->city_id : "" }}',
                academic_years_id: '{{ isset($application)? $application->academic_years_id : "" }}',
                grade: '{{ isset($application)? $application->grade : "" }}'
            }
        },
        methods: {
        },
...
//            }
    ...
    function showAzharDegree() {
      var x = document.getElementById("azharDegree");
        x.style.display = "contents";
    }
     function hideAzharDegree() {
      var x = document.getElementById("azharDegree");
        x.style.display = "none";
    }
