...
<table class="table table-bordered default-datatable" id="table" >
    ...
    <tr>
        <td>{!! $row->id !!}</td>
        <td>{!! $row->militaryArea->name !!}</td>
        <td>{!! $row->government->name !!}</td>
        <td>{!! $row->city->name !!}</td>
        <td>{!! $row->notes !!}</td>
        <td>
            <div class="btn-group" role="group" aria-label="Basic example">
                <a href="{{route('military-area-submission.edit', $row->id)}}">
                ...
                <form id="deleteForm{{$row->id}}" action="{{route('military-area-submission.destroy',$row->id)}}" method="post">
                    @method("DELETE")
                    @csrf
                </form>
            </div>
