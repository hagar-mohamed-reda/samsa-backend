<table class="table table-bordered default-datatable" id="table" >
    ...
    <tbody>
        ...
        <tr>
            <td>{!! $row->id !!}</td>
            <td>{!! $row->condition_age !!}</td>
            <td>{!! $row->militaryStatus->name !!}</td>
            <td>{!! $row->reason !!}</td>
            <td>{!! $row->notes !!}</td>
            <td>
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('military-status-reason.edit', $row->id)}}">
                    ...
                    <form id="deleteForm{{$row->id}}" action="{{route('military-status-reason.destroy',$row->id)}}" method="post">
                        @method("DELETE")
                        @csrf
                    </form>
                </div>
